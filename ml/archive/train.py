...
def load_and_split_data():
    df = pd.read_csv("data/prompt_examples_120 (1).csv") 
    X = df["prompt_text"] 
    Y = df["functional_type"] 
...
def save_mistakes(X_test, Y_test, Y_pred):
    ...
    for text, true_label, pred_label in zip(X_test.tolist(), Y_test.tolist(), Y_pred):
        if true_label != pred_label:
            ...
            mistakes_df = pd.DataFrame(mistakes, columns=["Prompt", "True Label", "Predicted Label"])
            mistakes_df.to_csv("model_mistakes.csv", index=False)

def save_model_vectorizer(model, vectorizer): 
        with open("ml/prompt_model.pkl", "wb") as model_file:
            pickle.dump(model, model_file)
        with open("ml/prompt_vectorizer.pkl", "wb") as vectorizer_file:
